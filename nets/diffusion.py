...
def changedata(i):
    if i <= 9:
        ...
        st = str(i)
    image = Image.open('D:\\JIT\pycharm\\CODE\\ddpm-pytorch-master\\gauss\\00' + st + '.png')  # <class 'PIL.Image.Image'>
    image = cvtColor(image).resize([32, 32], Image.BICUBIC)
    image = np.array(image, dtype=np.float32)  # (32, 32, 3) <class 'numpy.ndarray'>
    image = np.transpose(preprocess_input(image), (2, 0, 1))  # (3, 32, 32)
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, batch_size, device, y=None, use_ema=True):
        ...
        x = torch.randn(batch_size, self.img_channels, *self.img_size, device=device) # torch.Size([1, 3, 32, 32])
        #---------------------
        ...
        #                             drop_last=True, collate_fn=Diffusion_dataset_collate, sampler=None)



        # with open("train_lines.txt") as f:
        #     lines = f.readlines()
        # train_dataset = DiffusionDataset(lines, (32, 32))
        # # gen = DataLoader(train_dataset, shuffle=True, batch_size=1, num_workers=4,
        # #                  pin_memory=True,
        # #                  drop_last=True, collate_fn=Diffusion_dataset_collate, sampler=None)
        # xx = next(iter(train_dataset))

        if batch_size == 1:
            i = 1
            # image = Image.open('D:\\JIT\pycharm\\CODE\\ddpm-pytorch-master\\gauss\\0001.png') # <class 'PIL.Image.Image'>
            # image = cvtColor(image).resize([32, 32], Image.BICUBIC)
            # image = np.array(image, dtype=np.float32) # (32, 32, 3) <class 'numpy.ndarray'>
            # image = np.transpose(preprocess_input(image), (2, 0, 1)) # (3, 32, 32)
            # image = torch.tensor(image)
